...
CSRPVT=[5124529069]
global STCLOG
STCLOG= -1001936360090
#1001927160194
CSRLOG=-1001936360090
...
global rlink
rlink="https://t.me/AdvAIWorld/11"
...
pyro = Client("AdvChatGpt", api_id=API_ID, api_hash=API_KEY, bot_token=BOT_TOKEN)
...
ɴᴀᴍᴇ: [ᴀᴅᴠᴄʜᴀᴛɢᴘᴛ](https://t.me/AdvChatGpt_bot)
...
ꜱᴏᴜʀᴄᴇ ᴄᴏᴅᴇ: [ᴘʀɪᴠᴀᴛᴇ & © ᴀʟʟ ᴄᴏᴅᴇ ʀɪɢʜᴛꜱ ʀᴇꜱᴇʀᴠᴇᴅ ʙʏ ᴄꜱʀ.](https://t.me/I_AM_CSR)
...
"""
        ...
        reply_markup = InlineKeyboardMarkup(
            [
                ...
                [
                    InlineKeyboardButton("💬 ꜱᴜᴘᴘᴏʀᴛ", url="https://t.me/Shlok_21Q"),
                    InlineKeyboardButton("👨‍💼 ᴀᴅᴍɪɴ ", url="https://t.me/STC_TEAM_PALAK"),
                ],
                [
        ...
        if callback_query.data in ["high","medium","slow"]:
            if callback_query.data =="high":
                 ...
                 bun8+="✅"
        if bun6 !="High✅"and bun7 !="Medium✅" and  bun8!="Slow✅" :
            bun6+="✅"
    ...
    try:
        ...
        for i in range(len(msgs)):
            ...
            if i < images_count:
                ...
                await msg.edit(f"ᴄʀᴇᴀᴛᴇᴅ {i+1} ɪᴍᴀɢᴇꜱ ꜰᴏʀ {mention} !!  {status} "+PROMO,disable_web_page_preview=True)
        ...
        await client.send_message(chat_id=chat_id, text=f"ɪᴍᴀɢᴇ ɢᴇɴᴇʀᴀᴛᴏʀ : {e} {status}"+PROMO,disable_web_page_preview=True)
...
async def set_update_handle(bot, update):
    if update.chat.id not in SUPER_USER_IDS:
        await bot.send_message(chat_id=update.chat.id, text="Only CSR CAN !")
...
@pyro.on_message(filters.command("image") & filters.private )
async def set_suggest_mode_handle(bot, update):
    user_id = update.from_user.id
    user = update.from_user
    first_name = user.first_name
    ...
    if await user_is_member(user_id):
        ...
        github_link = "https://t.me/STC4INDIA"
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("JOIN STUDENTS COMMUNITY", url=github_link)],
        ])
        mention = user.mention(first_name)
        message_text = f"Dear {mention},\n"
        message_text+=FCRMSG
        await bot.send_message(chat_id=update.chat.id, text= message_text,
    ...
    if not check_inited(update.from_user.id):
        ...
        try:
            ...
            logger.exception(f"Failed to initialize for user [{update.from_user.id}]")
            del EDGES[update.from_user.id]
            reply_text = f"{bot_name}: Failed to initialize.({e}) \n**Contact admins at @STC_INDIA**"
            await update.reply(reply_text)
            return
        
    username = update.from_user.username 
    if not username:
        username = "AI_INDIA_BOT"
    user_id = update.from_user.id
    user = update.from_user
    first_name = user.first_name
    mention = user.mention(first_name)
    info =f"**User ➸ {mention}\nUserName ➸ @{username}\nUserID ➸ {user_id}\n\n**BOT  ➸ @AI_INDIA_BOT ( SERVER 01 )**"
    if update.from_user.id in CSRPVT:
        STCLOG=CSRLOG
    
    logger.info(f"Receive commands [{update.command}] from [{update.chat.id}]")
    if len(update.command) > 1:
        chat_id = update.chat.id
        op = await update.reply(text="Genrating Your Image.... ")
        prompt = " ".join(update.command[1:])
        defa="  Highly detailed,4K"
        prompt+=defa
        ...
        try:
            ...
            logger.exception(f"ImageGenerator Error: {e}")
            await bot.send_message(chat_id=chat_id, text=f"ImageGenerator Error: {e}.\n\nImageGenerator Usage: /image cute cats ")
            return
    await update.reply(text="ImageGenerator Usage: /image cute cats")



@pyro.on_message(filters.command("draw") & filters.private )
async def set_suggest_mode_handle(bot, update):
    user_id = update.from_user.id
    user = update.from_user
    first_name = user.first_name
    ...
    if len(update.command) > 1:
        ...
        try:
            ...
            logger.exception(f"ImageGenerator Error: {e}")
            await bot.send_message(chat_id=chat_id, text=f"ImageGenerator Error: {e}.\n\nImageGenerator Usage: /draw Cute Cats")
            return
    await update.reply(text="ImageGenerator Usage: /draw cute cats")
'''
...
# Define the ogg_to_wav function
...
@pyro.on_message(filters.voice & filters.private | filters.voice & filters.group)
async def handle_voice(bot, update):
    user_list=  load_members_users()
    premium_users = load_premium_users()
    admin_user =  load_admin()
    ...
    reply_text = f"**{bot_name} ɪꜱ ʀᴇᴀᴅɪɴɢ ᴠᴏɪᴄᴇ...✨**{status}"
    ...
    text = response
    try:
        text= text.replace("Bing", "Advance Chat Gpt")
        text =text.replace("Microsoft", "@AdvAIWorld")
    except Exception as e:
        text = response
        text = response.replace("Bing", "Advance Chat Gpt")
        text = text.replace("Microsoft", "@AdvAIWorld")
...
async def chat_handle(bot, update):
    ...
    try:
        response, reply_markup = await bingAI(update.from_user.id, update.text)
    except Exception as e:
        await bot.send_message(chat_id=STCLOG, text="**#Error_Raised**\n\n"+"Error:"+str(e)+"\n#USER_INFO \n"+info,disable_web_page_preview=True)
...
async def chat_handle(bot, update):
    ...
    print(question)
    try:
        response, reply_markup = await bingAI(update.from_user.id, question)
    except Exception as e:
        await bot.send_message(chat_id=STCLOG, text="**#Error_Raised**\n\n"+"Error:"+str(e)+"\n#USER_INFO \n"+info)
        await pop.edit(text="ꜱᴏʀʀʏ, ᴛʜᴇʀᴇ ɪꜱ ꜱᴏᴍᴇ ᴋɪɴᴅ ᴏꜰ ᴇʀʀᴏʀ \nɪ ʜᴀᴠᴇ ɪɴꜰᴏʀᴍᴇᴅ ᴛʜᴇ ᴛᴇᴀᴍ ᴛᴏ ꜰɪx ᴛʜɪꜱ ᴇʀʀᴏʀ.\nᴘʟᴇᴀꜱᴇ ᴛʀʏ ᴀꜰᴛᴇʀ ꜱᴏᴍᴇ ᴛɪᴍᴇ !"+PROMO)
        return
...
async def inline_query_image_gen_handle(bot, update):
    ...
    first_name = user.first_name
    mention = user.mention(first_name)
    info =f"\nDetails: User ➸ {mention}\nUserName ➸ @{username}\nUserID ➸ {user_id} \nUserStatus {status}"
    if update.from_user.id not in allow:
        logger.warning(f"User [{update.from_user.id}] try to send inline_query image_gen but been rejected (Not a premium user).")
        await update.answer(
            results=[
                InlineQueryResultArticle(
                    title="Not a Premium User !",
                    input_message_content=InputTextMessageContent(
                        f"ʏᴏᴜ ᴄᴀɴ'ᴛ ᴜꜱᴇ ᴛʜɪꜱ ꜰᴇᴀᴛᴜʀᴇ,ʙᴇ ᴀ ᴘʀᴇᴍɪᴜᴍ ᴜꜱᴇʀ ᴛᴏ ᴜꜱᴇ ᴛʜɪꜱ ꜰᴇᴀᴛᴜʀᴇ.\n\n**ʏᴏᴜ ᴄᴀɴ ᴀʟꜱᴏ ᴜꜱᴇ ɪᴍᴀɢᴇ ɢᴇɴᴇʀᴀᴛɪᴏɴ ɪɴ  @AdvAIWorld ɢʀᴏᴜᴘ ꜰᴏʀ ꜰʀᴇᴇ.**{PROMO}"
                    ),
                    description="You can't use this feature,Be a premium user to use this feature.\nPowered by @AdvAIWorld",
                )
            ],
            cache_time=1
        )
    ...
    if not check_inited(update.from_user.id):
        ...
        await update.answer(
            results=[
                InlineQueryResultArticle(
                    title="First send /start command to use me .",
                    input_message_content=InputTextMessageContent(
                        "Not initialized"
                    ),
                    description="ᴛᴏ ᴇɴᴀʙʟᴇ ᴛʜɪꜱ ꜰᴇᴀᴛᴜʀᴇ ꜰɪʀꜱᴛ ꜱᴇɴᴅ /start or /new ᴄᴏᴍᴍᴀɴᴅ ᴀɴᴅ ᴛʜᴇɴ ᴛʀʏ ᴀɢᴀɪɴ ᴛʜɪꜱ ꜰᴜɴᴄᴛɪᴏɴ.",
                )
            ],
            cache_time=1
        )
        ...
        await update.answer(
            results=[
                InlineQueryResultArticle(
                    ...
                    input_message_content=InputTextMessageContent(
                        "Please add '%' at the end of prompt to confirm the prompt. Add more (Max: 4) '%' to select next image. (Tips: If it take long time (25s) no response, you can add a '%' and delete it to refresh)"+PROMO
                    )
                )
            ]
        )
    ...
    try:
        ...
        await bot.send_message(chat_id=STCLOG, text="#inlineQuery\n"+prompt+info,disable_web_page_preview=True)

        
        await update.answer(
            results=[
                InlineQueryResultPhoto(
                    title="ImageGeneration AdvChatGpt Prompt: " + prompt,
                    description="Add more '%' at the end of prompt to select next image(AdvChatGpt)",
                    photo_url=img_url,
                    caption=f"ᴛʜɪꜱ ɪᴍᴀɢᴇ ɪꜱ ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ **@AdvChatGpt_bot** ᴜꜱɪɴɢ ᴘʀᴏᴍᴘᴛ : {prompt}"+PROMO,
                )
            ],
            cache_time=1
        )
